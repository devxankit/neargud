...
const MobileFlashSale = () => {
  ...
  const loadMore = () => {
    if (!loading && hasMore) fetchData(page + 1, true);
  };
        ...
        <div className="w-full pb-24">
          <div className="px-4 py-4 bg-gradient-to-r from-red-50 to-orange-50 border-b border-gray-200 sticky top-1 z-30">
            <div className="flex items-center gap-3 mb-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-gray-600">{products.length} items on sale</p>
              </div>
                    ...
                    {showFilters && (
                      <>
                        ...
                        <motion.div initial={{ opacity: 0, y: -10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -10, scale: 0.95 }} className="filter-dropdown absolute right-0 top-full w-56 bg-white rounded-xl shadow-2xl z-[10001] overflow-hidden" style={{ marginTop: "-50px" }}>
                          <div className="p-3 border-b bg-gray-50 flex justify-between items-center font-bold text-sm">Filters <button onClick={() => setShowFilters(false)}><FiX /></button></div>
                          <div className="p-3 space-y-3">
                            <input type="number" placeholder="Min" value={filters.minPrice} onChange={e => handleFilterChange('minPrice', e.target.value)} className="w-full border p-2 rounded text-xs" />
                            <input type="number" placeholder="Max" value={filters.maxPrice} onChange={e => handleFilterChange('maxPrice', e.target.value)} className="w-full border p-2 rounded text-xs" />
                          </div>
          ...
          <div className="px-4 py-4">
            {products.length === 0 && !loading ? (
              <div className="text-center py-12"><div className="text-6xl mx-auto mb-4">⚡</div><h3 className="text-xl font-bold mb-2">No flash sale items</h3></div>
            ) : viewMode === 'grid' ? (
              <>
                <div className="grid grid-cols-2 gap-3">
                  {products.map((p, i) => <motion.div key={p._id || p.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}><ProductCard product={p} /></motion.div>)}
              ...
              <div className="space-y-3">
                {products.map((p, i) => <ProductListItem key={p._id || p.id} product={p} index={i} />)}
