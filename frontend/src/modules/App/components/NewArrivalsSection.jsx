...
const NewArrivalsSection = ({ products = [], loading = false, theme = null }) => {
  ...
  return (
    <motion.div
      ...
      className={`relative mx-4 my-4 rounded-t-2xl rounded-b-[40px] md:rounded-2xl overflow-hidden shadow-xl border-2 transition-all duration-500 md:mx-4 lg:mx-8 max-w-screen-2xl xl:mx-auto ${!theme ? "border-cyan-200 bg-gradient-to-br from-cyan-500 via-blue-500 to-indigo-500" : ""}`}
      style={theme ? {
          ...
          {loading ? (
            ...
            newArrivals.map((product, index) => {
              ...
              return (
                ...
                >
                  <motion.div
                    className="rounded-t-xl rounded-b-3xl overflow-hidden h-full"
                  >
                    <Link to={productLink} className="group flex flex-col h-full">
                      <div className="relative rounded-t-xl rounded-b-3xl overflow-hidden border border-white/20 transition-all duration-300 flex-1 flex flex-col">
