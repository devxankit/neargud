...
import { FiPlus, FiSearch, FiEdit, FiTrash2, FiTag, FiCopy, FiCheck, FiX } from 'react-icons/fi';
...
const Promotions = () => {
  ...
  const filteredPromotions = promotions.filter((promo) => {
    const matchesSearch = !searchQuery || promo.name?.toLowerCase().includes(searchQuery.toLowerCase()) || promo.code?.toLowerCase().includes(searchQuery.toLowerCase());
    const matchesStatus = statusFilter === 'all' || promo.status === statusFilter;
    return matchesSearch && matchesStatus;
  });

  const handleSave = (promoData) => {
    const updatedPromos = editingPromo && editingPromo.id
      ? promotions.map((p) => (p.id === editingPromo.id ? { ...promoData, id: editingPromo.id, vendorId } : p))
      : [...promotions, { ...promoData, id: Date.now(), vendorId, usageCount: 0 }];

    setPromotions(updatedPromos);
    localStorage.setItem(`vendor-${vendorId}-promotions`, JSON.stringify(updatedPromos));
    setEditingPromo(null);
    setShowForm(false);
    toast.success(editingPromo && editingPromo.id ? 'Promotion updated' : 'Promotion added');
  };
  ...
  const columns = [
        ...
        <div>
          ...
          {row.code && (
            <div className="flex items-center gap-2 mt-1">
              <code className="text-xs bg-gray-100 px-2 py-1 rounded">{row.code}</code>
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  handleCopy(row.code);
    ...
    },
    {
      ...
      sortable: true,
      render: (value) => (
        <Badge variant="info">{value === 'discount' ? 'Discount' : value === 'flash_sale' ? 'Flash Sale' : 'Promo Code'}</Badge>
      ),
    },
    {
      key: 'discountValue',
    ...
    <motion.div
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="lg:hidden">
          ...
          <p className="text-sm sm:text-base text-gray-600">Manage discounts and promotional offers</p>
        </div>
        <button
          onClick={() => {
            setEditingPromo(null);
          ...
          <div className="w-full sm:w-auto">
            <ExportButton
              ...
              headers={[
                ...
                { label: 'Discount', accessor: (row) => row.discountType === 'percentage' ? `${row.discountValue}%` : formatPrice(row.discountValue) },
                { label: 'Status', accessor: (row) => row.status },
                { label: 'Usage', accessor: (row) => `${row.usageCount || 0} / ${row.usageLimit || '∞'}` },
              ]}
    ...
    <div className="fixed inset-0 bg-black/50 z-[10000] flex items-center justify-center p-4">
      <div className="bg-white rounded-xl p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-semibold text-gray-600 mb-2">Discount Type *</label>
            <select
          ...
          <div>
            <label className="block text-sm font-semibold text-gray-600 mb-2">Discount Value *</label>
            <input
              type="number"
