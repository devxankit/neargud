...
import { FiMapPin, FiPlus, FiEdit, FiTrash2, FiSearch, FiCheckCircle, FiX } from 'react-icons/fi';
...
const PickupLocations = () => {
  ...
  useEffect(() => {
    ...
    const savedLocations = localStorage.getItem(`vendor-${vendorId}-pickup-locations`);
    if (savedLocations) {
      ...
      if (vendor.address) {
        ...
        localStorage.setItem(`vendor-${vendorId}-pickup-locations`, JSON.stringify([defaultLocation]));
      }
    }
  }, [vendorId, vendor]);
  ...
  const handleSave = (locationData) => {
    const updatedLocations = locationModal.location?.id
      ? locations.map((l) => (l.id === locationModal.location.id ? { ...locationData, id: locationModal.location.id } : l))
      : [...locations, { ...locationData, id: Date.now() }];

    setLocations(updatedLocations);
    localStorage.setItem(`vendor-${vendorId}-pickup-locations`, JSON.stringify(updatedLocations));
    setLocationModal({ isOpen: false, location: null });
    toast.success(locationModal.location?.id ? 'Location updated' : 'Location added');
  };

  const handleDelete = () => {
    const updatedLocations = locations.filter((l) => l.id !== deleteModal.locationId);
    setLocations(updatedLocations);
    localStorage.setItem(`vendor-${vendorId}-pickup-locations`, JSON.stringify(updatedLocations));
  ...
  if (!vendorId) {
    return (
      <div className="text-center py-12">
        <p className="text-gray-500">Please log in to manage pickup locations</p>
      </div>
    );
  }
    ...
    <motion.div
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="lg:hidden">
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-800 mb-2">Pickup Locations</h1>
          <p className="text-sm sm:text-base text-gray-600">Manage your store pickup locations</p>
        </div>
        <button
