    ...
    >
      ...
      >
        ...
        {showLogo && (vendor.storeLogo || vendor.logo) ? (
          <div className={`${logoSizeClasses[size]} rounded-full overflow-hidden bg-white border border-primary-200 flex-shrink-0`}>
            <LazyImage
          ...
          <FiShoppingBag className={`text-primary-600 ${size === 'sm' ? 'text-[10px]' : size === 'md' ? 'text-xs' : 'text-sm'}`} />
        )}
        ...
        {showVerified && vendor.isVerified && (
          ...
          >
            <FiCheckCircle className={`text-accent-600 ${size === 'sm' ? 'text-[10px]' : size === 'md' ? 'text-xs' : 'text-sm'}`} title="Verified Vendor" />
          </motion.div>
        )}
      </motion.div>
    </span>
