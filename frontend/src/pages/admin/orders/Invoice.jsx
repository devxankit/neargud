...
Items:
${items.map(item => `- ${item.name || 'Item'} x${item.quantity || 1} - ${formatCurrency((item.price || 0) * (item.quantity || 1))}`).join('\n')}
              ...
              {items.map((item, index) => {
                ...
                return (
                  <tr key={item.id || index} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-700 text-right">{formatCurrency(item.price || 0)}</td>
                    <td className="px-4 py-3 text-sm font-semibold text-gray-800 text-right">{formatCurrency(itemTotal)}</td>
                  </tr>
                );
              })}
            </tbody>
