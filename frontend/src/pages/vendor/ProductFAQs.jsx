...
import { FiPlus, FiEdit, FiTrash2, FiChevronDown, FiChevronUp, FiHelpCircle } from 'react-icons/fi';
...
const ProductFAQs = () => {
  ...
  const handleSave = (faqData) => {
    const updatedFaqs = editingFaq && editingFaq.id
      ? faqs.map((f) => (f.id === editingFaq.id ? { ...faqData, id: editingFaq.id, vendorId } : f))
      : [...faqs, { ...faqData, id: Date.now(), vendorId }];
    
    setFaqs(updatedFaqs);
    localStorage.setItem(`vendor-${vendorId}-faqs`, JSON.stringify(updatedFaqs));
    setEditingFaq(null);
      ...
      <div className="space-y-3">
        {filteredFaqs.length > 0 ? (
          filteredFaqs.map((faq) => (
            <div key={faq.id} className="bg-white rounded-xl p-4 shadow-sm border border-gray-200">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-2">
                    <p className="text-sm font-semibold text-gray-600">
                      {products.find((p) => p.id === faq.productId)?.name || 'Unknown Product'}
                    </p>
                  </div>
                  <button
                  ...
                  <AnimatePresence>
                    {expandedFaq === faq.id && (
                      <motion.div
                        ...
                        className="mt-2">
                        <p className="text-sm text-gray-600 bg-gray-50 p-3 rounded-lg">{faq.answer}</p>
                      </motion.div>
                    )}
                  </AnimatePresence>
                </div>
    ...
    <div className="fixed inset-0 bg-black/50 z-[10000] flex items-center justify-center p-4">
      <div className="bg-white rounded-xl p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-bold text-gray-800">{faq?.id ? 'Edit FAQ' : 'Add FAQ'}</h3>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-semibold text-gray-600 mb-2">Answer *</label>
            <textarea
