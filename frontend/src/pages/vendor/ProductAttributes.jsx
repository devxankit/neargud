...
const ProductAttributes = () => {
  ...
  const handleSave = (attrData) => {
    const updated = editingAttr && editingAttr.id
      ? attributes.map((a) => (a.id === editingAttr.id ? { ...attrData, id: editingAttr.id, vendorId } : a))
      : [...attributes, { ...attrData, id: Date.now(), vendorId }];
    setAttributes(updated);
    localStorage.setItem(`vendor-${vendorId}-attributes`, JSON.stringify(updated));
    setEditingAttr(null);
    ...
    },
    {
      ...
      render: (_, row) => (
        <div className="flex gap-2">
          <button onClick={() => setEditingAttr(row)} className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg"><FiEdit /></button>
    ...
    <motion.div
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="lg:hidden">
          ...
          <p className="text-sm sm:text-base text-gray-600">Manage product attributes and values</p>
        </div>
        <button
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-[10000] flex items-center justify-center p-4">
      <div className="bg-white rounded-xl p-6 max-w-md w-full">
        <h3 className="text-lg font-bold mb-4">{attribute?.id ? 'Edit Attribute' : 'Add Attribute'}</h3>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-semibold mb-2">Values (comma-separated)</label>
            <input
              type="text"
