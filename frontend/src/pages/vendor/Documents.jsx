...
import { FiFile, FiUpload, FiDownload, FiTrash2, FiCheckCircle, FiXCircle } from 'react-icons/fi';
...
const Documents = () => {
  ...
  const handleUpload = (docData) => {
    const updated = [...documents, { ...docData, id: Date.now(), vendorId, uploadedAt: new Date().toISOString(), status: 'pending' }];
    setDocuments(updated);
    localStorage.setItem(`vendor-${vendorId}-documents`, JSON.stringify(updated));
    setShowUpload(false);
  ...
  const columns = [
    ...
    { key: 'category', label: 'Category', sortable: true },
    {
      ...
      label: 'Status',
      render: (value) => (
        <Badge variant={value === 'approved' ? 'success' : value === 'rejected' ? 'error' : 'warning'}>
          {value}
        </Badge>
      ),
    },
    ...
    },
    {
      ...
      render: (_, row) => (
        <div className="flex gap-2">
          <button className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg"><FiDownload /></button>
          <button onClick={() => setDeleteModal({ isOpen: true, id: row.id })} className="p-2 text-red-600 hover:bg-red-50 rounded-lg"><FiTrash2 /></button>
        </div>
      ),
    },
  ];
    ...
    <motion.div
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="lg:hidden">
          ...
          <p className="text-sm sm:text-base text-gray-600">Manage business documents and certificates</p>
        </div>
        <button
      ...
      <div className="bg-white rounded-xl p-6 max-w-md w-full">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex justify-end gap-2">
            ...
            <button type="submit" className="px-4 py-2 bg-primary-600 text-white rounded-lg">Upload</button>
          </div>
        </form>
      </div>
    </div>
