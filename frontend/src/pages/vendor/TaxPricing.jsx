...
const TaxPricing = () => {
  ...
  const [bulkAction, setBulkAction] = useState({ type: 'percentage', value: 0 });
  ...
  const handleBulkUpdate = () => {
    ...
    const updated = products.map((p) => {
      if (selectedProducts.includes(p.id)) {
        const newPrice = bulkAction.type === 'percentage'
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (value, row) => {
        ...
        return <span className="font-semibold text-green-600">{formatPrice(value + tax)}</span>;
      },
    },
  ];
  ...
  return (
    <motion.div
      ...
      <div className="lg:hidden">
        ...
        <p className="text-sm sm:text-base text-gray-600">Manage product prices and tax rates</p>
      </div>
      ...
      <div className="bg-white rounded-xl p-4 sm:p-6 shadow-sm border border-gray-200">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
          ...
          <input
            ...
            placeholder={bulkAction.type === 'percentage' ? 'Enter percentage' : 'Enter amount'}
