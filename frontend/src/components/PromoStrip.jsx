...
const getCategoryIcons = (categoryId) => {
    const iconMap = {
        1: ['👕', '👗', '🧥', '👔'], // Clothing
        2: ['👠', '👟', '🥿', '👢'], // Footwear
        3: ['👜', '💼', '🎒', '🛍️'], // Bags
...
const getFeaturedProducts = (activeTab, categoryId) => {
...
const getDynamicCategoryCards = (categories, categoryProductsMap) => {
...
const getCategoryCards = (activeTab, categoryId, dynamicCategories, categoryProductsMap) => {
...
const rgbToRgba = (rgbString, opacity) => {
...
export default function PromoStrip({
    ...
    const crazyDealsText = {
        line1: activeBanner?.description?.split(' ')[0] || promoStripCMS?.crazyDealsText?.line1 || 'CRAZY',
        line2: activeBanner?.description?.split(' ').slice(1).join(' ') || promoStripCMS?.crazyDealsText?.line2 || 'DEALS',
    };
    ...
    const housefullRef = useRef(null);
    ...
    useLayoutEffect(() => {
        ...
        const ctx = gsap.context((self) => {
            ...
            if (housefullContainer) {
                ...
                if (targets.length > 0) {
                    tl.set(targets, { scale: 1, opacity: 1 })
                        .to(targets, {
                            scale: 0,
                            opacity: 0,
                            duration: 0.6,
