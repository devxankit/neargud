...
const BannerForm = ({ banner, onClose, onSave }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      bannerPayload.append("type", formData.type);
      bannerPayload.append("title", formData.title);
      bannerPayload.append("subtitle", formData.subtitle);
      bannerPayload.append("description", formData.description);
      bannerPayload.append("link", formData.link);
      bannerPayload.append("order", formData.order);
      bannerPayload.append("isActive", formData.isActive);

      if (formData.startDate) {
        bannerPayload.append("startDate", new Date(formData.startDate).toISOString());
      }
      if (formData.endDate) {
        bannerPayload.append("endDate", new Date(formData.endDate).toISOString());
      }

      if (selectedFile) {
        bannerPayload.append("image", selectedFile);
      }

      if (isEdit) {
        await updateBanner(banner.id || banner._id, bannerPayload);
      } else {
