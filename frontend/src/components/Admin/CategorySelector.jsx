...
const CategorySelector = ({ value, subcategoryId, onChange, required = false, className = '' }) => {
  const { categories, getRootCategories, getCategoriesByParent, getCategoryById } = useCategoryStore();
  ...
  const selectedSubcategory = subcategoryId ? getCategoryById(subcategoryId) : null;
  const parentCategory = selectedSubcategory ? getCategoryById(selectedSubcategory.parentId) : selectedCategory;

  // Get subcategories for hovered category
  const hoveredSubcategories = useMemo(() => {
    if (!hoveredCategoryId) return [];
    return getCategoriesByParent(hoveredCategoryId).filter(cat => cat.isActive !== false);
  ...
  return (
    <div ref={containerRef} className={`relative ${className}`}>
      ...
      >
        ...
        <FiChevronDown
          className={`ml-2 text-gray-500 transition-transform ${isOpen ? 'transform rotate-180' : ''}`}
        />
      </button>
              ...
              <div className="py-1">
                {rootCategories.length === 0 ? (
                  ...
                  rootCategories.map((category) => {
                    const subcategories = getCategoriesByParent(category.id).filter(cat => cat.isActive !== false);
