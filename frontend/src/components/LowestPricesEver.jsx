...
const ProductCard = memo(({
    ...
    return (
        <motion.div
            whileHover={{ y: -2 }}
            className="flex-shrink-0 w-[140px] md:w-[180px] lg:w-[220px]"
                ...
                <div onClick={() => navigate(`/app/product/${productId}`)} className="relative block cursor-pointer">
                    <div className="w-full h-28 md:h-36 lg:h-44 bg-neutral-100 flex items-center justify-center overflow-hidden relative">
                        ...
                        <button
                            ...
                            className="absolute top-1 right-1 z-10 w-5 h-5 md:w-7 md:h-7 rounded-full backdrop-blur-sm flex items-center justify-center transition-colors shadow-sm bg-white/95 text-neutral-700"
                            style={isFavorite ? { color: theme.accentColor, backgroundColor: hexToRgba(theme.accentColor, 0.1) } : {}}
                        >
                            <FiHeart size={10} className="md:w-3.5 md:h-3.5" fill={isFavorite ? "currentColor" : "none"} />
                        </button>
                    </div>
                </div>

                <div className="p-1.5 md:p-3 flex-1 flex flex-col" style={{ background: cardBgColor }}>
                    <div className="flex gap-0.5 mb-0.5 md:mb-1.5">
                    ...
                    {product.rating > 0 && product.reviewCount > 0 && (
                        <div className="flex items-center gap-0.5 mb-0.5 md:mb-1.5">
                            <FiStar size={8} className="md:w-3 md:h-3 text-yellow-400 fill-yellow-400" />
                            <span className="text-[8px] md:text-xs text-neutral-500">({product.reviewCount})</span>
                        </div>
                    )}

                    <div className="flex items-center justify-between mt-auto pt-1">
                        <div className="flex flex-col">
                            <span className="text-[11px] md:text-sm lg:text-lg font-bold text-neutral-900">₹{product.price}</span>
                            {originalPrice > product.price && <span className="text-[8px] md:text-[10px] text-neutral-400 line-through">₹{originalPrice}</span>}
                        </div>
                    </div>
                    ...
                    >
                        <span>See more</span>
                        <svg width="6" height="6" className="md:w-2 md:h-2" viewBox="0 0 8 8" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M0 0L8 4L0 8Z" fill={theme.accentColor} /></svg>
                    </div>
                </div>
            </div>
        </motion.div>
...
export default function LowestPricesEver({ activeTab = 'all', categoryId, products = [], loading = false }) {
    ...
    return (
        <div className="relative w-full overflow-x-hidden pt-6 pb-6 mt-2 md:pt-10 md:pb-12" style={{
            background: `linear-gradient(to bottom, transparent 0%, ${theme.primary[3]} 15%, ${theme.secondary[2]} 100%)`,
        }}>
            <div className="px-4 md:px-8 lg:px-12 relative z-20 mt-4 mb-3 md:mb-6 max-w-screen-2xl mx-auto">
                <div className="flex items-center gap-2 md:gap-4">
                    <div className="flex-1 h-px" style={{ backgroundColor: '#000', opacity: 0.2 }}></div>
                    <h2 className="font-black text-[24px] md:text-3xl lg:text-4xl text-center" style={{
            ...
            <div ref={scrollContainerRef} className="flex gap-4 md:gap-4 overflow-x-auto scrollbar-hide px-4 md:px-8 lg:px-12 max-w-screen-2xl mx-auto" style={{ scrollSnapType: 'x mandatory' }}>
                {loading ? [1, 2, 3, 4, 5, 6].map(i => <div key={i} className="flex-shrink-0 w-[140px] md:w-[180px] lg:w-[220px] bg-white rounded-lg h-48 md:h-64 animate-pulse shadow-sm" />) : discountedProducts.map((product) => (
                    <ProductCard
